...
BOOLEAN VmxDpcBroadcastAllocateVmxonRegions(KDPC* Dpc, PVOID DeferredContext, PVOID SystemArgument1, PVOID SystemArgument2)
{
	...
	{
		//LogInfo("Error in allocating memory for Vmxon region");
		return FALSE;
	}
	if (!VmxAllocateVmcsRegion(&GuestState[CurrentProcessorNumber]))
	{
		//LogInfo("Error in allocating memory for Vmcs region");
		return FALSE;
	}
...
BOOLEAN VmxAllocateVmxonRegion(VIRTUAL_MACHINE_STATE* CurrentGuestState)
{
	...
	if (VmxonRegion == NULL) {
		//LogInfo("Couldn't Allocate Buffer for VMXON Region.");
		return FALSE;
	}
	...
	*(UINT64*)AlignedVmxonRegion = VmxBasicMsr.Fields.RevisionIdentifier;
	...
	if (VmxonStatus)
	{
		//LogInfo("Executing Vmxon instruction failed with status : %d", VmxonStatus);
		return FALSE;
	}
...
BOOLEAN VmxAllocateVmcsRegion(VIRTUAL_MACHINE_STATE* CurrentGuestState)
{
	...
	if (VmcsRegion == NULL) {
		//LogInfo("Couldn't Allocate Buffer for VMCS Region.");
		return FALSE;
	}
...
BOOLEAN VmxAllocateVmmStack(INT ProcessorID)
{
	...
	{
		//LogInfo("Insufficient memory in allocationg Vmm stack");
		return FALSE;
	}
...
BOOLEAN VmxAllocateMsrBitmap(INT ProcessorID)
{
	...
	{
		//LogInfo("Insufficient memory in allocationg Msr bitmaps");
		return FALSE;
	}
	...
	{
		//LogInfo("Invalid parameters sent to the HvSetMsrBitmap function");
		return FALSE;
	}
	*/
