...
BOOLEAN VmxInitializer()
{
	...
	if (!HvIsVmxSupported())
	{
		//LogInfo("VMX is not supported in this machine !");
		return FALSE;
	}
	...
	if (!EptCheckFeatures())
	{
		//LogInfo("Your processor doesn't support all EPT features");
		return FALSE;
	}
	else
	{
		...
		if (!EptBuildMtrrMap())
		{
			//LogInfo("Could not build Mtrr memory map");
			return FALSE;
		}
	...
	{
		//LogInfo("Could not initialize pool manager");
		return FALSE;
	}
...
BOOLEAN VmxVirtualizeCurrentSystem(PVOID GuestStack)
{
	...
	//LogInfo("VMLAUNCH Error : 0x%llx", ErrorCode);

	//LogInfo("VMXOFF Executed Successfully but it was because of an error.");
...
VOID VmxVmresume()
{
	...
	// prefer to break
	//LogInfo("Error in executing Vmresume , status : 0x%llx", ErrorCode);
