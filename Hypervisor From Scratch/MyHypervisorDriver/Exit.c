...
BOOLEAN VmxVmexitHandler(PGUEST_REGS GuestRegs)
{
	...
	{
		LogInfo("Exit reason for I/O instructions are not supported yet.");
	...
	case EXIT_REASON_EPT_VIOLATION:
	{
		...
		curr_GuestRegs = GuestRegs;
		if (!EptHandleEptViolation(ExitQualification, GuestPhysicalAddr))
		{
			LogInfo("There were errors in handling Ept Violation");
		}
	...
	case EXIT_REASON_EXCEPTION_NMI:
	{
		...
		*/
		...
		{
			LogInfo("Not expected event occured");
		}
		break;
	}
