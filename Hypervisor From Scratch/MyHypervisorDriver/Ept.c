...
BOOLEAN EptSplitLargePage(PVMM_EPT_PAGE_TABLE EptPageTable, PVOID PreAllocatedBuffer, SIZE_T PhysicalAddress, ULONG CoreIndex)
{
	...
	NewPointer.PageFrameNumber = (SIZE_T)myVirtualAddressToPhysicalAddress(&NewSplit->PML1[0]) / PAGE_SIZE;
...
PVMM_EPT_PAGE_TABLE EptAllocateAndCreateIdentityPageTable()
{
	...
	PageTable->PML4[0].PageFrameNumber = (SIZE_T)myVirtualAddressToPhysicalAddress(&PageTable->PML3[0]) / PAGE_SIZE;
	...
	for (EntryIndex = 0; EntryIndex < VMM_EPT_PML3E_COUNT; EntryIndex++)
	{
		...
		PageTable->PML3[EntryIndex].PageFrameNumber = (SIZE_T)myVirtualAddressToPhysicalAddress(&PageTable->PML2[EntryIndex][0]) / PAGE_SIZE;
	}
...
BOOLEAN EptLogicalProcessorInitialize()
{
	...
	EPTP.PageFrameNumber = (SIZE_T)myVirtualAddressToPhysicalAddress(&PageTable->PML4) / PAGE_SIZE;
...
BOOLEAN EptHandleHookedPage(EPT_HOOKED_PAGE_DETAIL* HookedEntryDetails, VMX_EXIT_QUALIFICATION_EPT_VIOLATION ViolationQualification, SIZE_T PhysicalAddress) {
	...
	if (PsGetCurrentProcessId() == myProcessID && ExactAccessedAddress == myAddressToHook)
	{
		if (!ViolationQualification.EptExecutable && ViolationQualification.ExecuteAccess)
		{
			if (hookType == 1)
				Log("RIP: 0x%llx, RAX: 0x%llx, RBX: 0x%llx, RCX: 0x%llx, RDX: 0x%llx, RBP: 0x%llx, RSP: 0x%llx, RSI: 0x%llx, RDI: 0x%llx"
			, GuestRip, curr_GuestRegs->rax, curr_GuestRegs->rbx, curr_GuestRegs->rcx, curr_GuestRegs->rdx, curr_GuestRegs->rbp, curr_GuestRegs->rsp, curr_GuestRegs->rsi, curr_GuestRegs->rdi); 
			else if (hookType == 2) // my position
				LogMyAddress("%llx", curr_GuestRegs->rcx);
			else if (hookType == 3 && curr_GuestRegs->rdi == 0) { // players' position
				LogPlayersAddresses("%llx", curr_GuestRegs->rcx + 0x10);
			}
		}
...
BOOLEAN EptPerformPageHook(PVOID TargetAddress, PVOID HookFunction, PVOID* OrigFunction, BOOLEAN UnsetRead, BOOLEAN UnsetWrite, BOOLEAN UnsetExecute) {
	...
	{	
		LogError("This function should be called in VMX Non-Root Mode");
		return FALSE;
	}
	...
	PhysicalAddress = (SIZE_T)myVirtualAddressToPhysicalAddress(VirtualTarget);
	if (!PhysicalAddress)
	{
	...
	HookedPage->PhysicalBaseAddressOfFakePageContents = (SIZE_T)myVirtualAddressToPhysicalAddress(&HookedPage->FakePageContents[0]) / PAGE_SIZE;
	...
	entryAddressToRecover = TargetPage;
	entryValueToRecover = *TargetPage;
...
BOOLEAN EptPageHook(PVOID TargetAddress, PVOID HookFunction, PVOID* OrigFunction, BOOLEAN SetHookForRead, BOOLEAN SetHookForWrite, BOOLEAN SetHookForExec) {
	...
	if (FALSE && GuestState[LogicalCoreIndex].HasLaunched)
	{
		...
		if (AsmVmxVmcall(VmcallNumber, TargetAddress, HookFunction, OrigFunction) == STATUS_SUCCESS)
		{
			...
			LogWarning("Hook not applied from VMX Root Mode");
			return FALSE;
		}
	}
	else
	{	
		LogInfo("Hook applying from VMX Non-Root Mode");
