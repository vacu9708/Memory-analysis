...
NTSTATUS DrvCreate(PDEVICE_OBJECT DeviceObject, PIRP Irp)
{
	...
	//else
	//{
	//	LogInfo("Hypervisor From Scratch was not loaded :(");
	//}
	...
	if (AllowIOCTLFromUsermode)
	{
		...
		case IOCTL_REGISTER_EVENT:
			...
			if (IrpStack->Parameters.DeviceIoControl.InputBufferLength < SIZEOF_REGISTER_EVENT || Irp->AssociatedIrp.SystemBuffer == NULL) {
				...
				LogInfo("Invalid parameter to IOCTL Dispatcher.");
				break;
			}
		...
		case MY_START:
			...
			PPROCESS_VADDR process_vaddr = (PPROCESS_VADDR)Irp->AssociatedIrp.SystemBuffer;
			if (inBufLength < sizeof(PROCESS_VADDR) || process_vaddr == NULL)
			{
				Status = STATUS_INVALID_PARAMETER;
				LogWarning("IOCTL_SET_PROCESS_VADDR: Invalid input buffer.");
				break;
			}
			// Set processID
			myProcessID = process_vaddr->processID;
			myDrvStart();
			// Set a hook if the virtual address is valid
			KAPC_STATE apcState;
			PEPROCESS targetProcess = NULL;
			myAddressToHook = process_vaddr->VirtualAddress;
			if (!NT_SUCCESS(PsLookupProcessByProcessId(myProcessID, &targetProcess))) {
				LogWarning("IOCTL_SET_PROCESS_VADDR: Invalid process ID.");
				break;
			}
			KeStackAttachProcess(targetProcess, &apcState);
			ULONG64 isValid = MmIsAddressValid(myAddressToHook);
			KeUnstackDetachProcess(&apcState);
			ObDereferenceObject(targetProcess);
			if (!isValid) {
				LogWarning("IOCTL_SET_PROCESS_VADDR: Invalid virtual address.");
				Status = STATUS_INVALID_PARAMETER;
				break;
			}
			hookType = process_vaddr->type;
			EptPageHook(myAddressToHook, NULL, NULL, TRUE, TRUE, FALSE);
			// LogInfo("IOCTL_SET_PROCESS_VADDR: ProcessID = %d, VirtualAddress = 0x%llx", myProcessID, myAddressToHook);
			Status = STATUS_SUCCESS;
			break;
		default:
